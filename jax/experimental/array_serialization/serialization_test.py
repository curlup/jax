...
class CheckpointTest(jtu.JaxTestCase):
  ...
  def test_maybe_cloud_storage(self):
    ...
    self.assertTrue(serialization._maybe_cloud_storage(gs_spec))

    local_path = '/tmp/checkpoint'
    local_spec = serialization.get_tensorstore_spec(local_path, ocdbt=True)
    self.assertFalse(serialization._maybe_cloud_storage(local_spec))
    ...
    self.assertTrue(serialization._maybe_cloud_storage(nested_tspec))
