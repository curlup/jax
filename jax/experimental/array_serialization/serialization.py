...
def _maybe_cloud_storage(tspec: Union[dict[str, Any], str]) -> bool:
  """Detect if user specifies using cloud storages.

  This only supports common usage and unable to detect some corner cases such as
  using gcsfuse.
  """
  ...
  for key in ('base', 'kvstore'):
    if key in tspec:
      return _maybe_cloud_storage(tspec[key])
...
async def async_serialize(
  ...
  if primary_host is None and _maybe_cloud_storage(tensorstore_spec):
    raise ValueError(
